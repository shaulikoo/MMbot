...
class ImageVisualFeatures:
    def __init__(self, feature_type, image1=None, image2=None):
        try:
            if isinstance(feature_type, ImageVisualFeatures):
    ...
    def dup_image_feature(self, image_feature2dup):
        image_feature = copy.deepcopy(image_feature2dup)
        self.feature_type = image_feature.feature_type
        self.descriptor = image_feature.descriptor
        self.weights = image_feature.weights
        self.feature = image_feature.feature
        ...
        if image1 is not None:
            ...
            if self.feature_type == "SIFT":
                if image2 is not None:  # we have image pair
                    ...
                    kp, descriptors = \
                        ImageVisualFeatures.keep_points_features_from_image_pair(self, kp_pairs, descriptors_pairs,
                                                                                 matches)
                else:
                    kp, descriptors = features_extractor.get_sift_points_and_features(image1)
                for p in kp:
                    # self.feature.append(PointFeature(p))
                    self.feature = [PointFeature(p) for p in kp]
    ...
    def concatenate_features(self, image_feature_2_concatenate):
        if image_feature_2_concatenate.descriptor is None or len(image_feature_2_concatenate.descriptor) == 0:
            return
    ...
    def compare_with_feature(self, feature_to_compare, min_matches_needed=0):
        ...
        if len(matches) < min_matches_needed * len(self.feature) or not matches:
            ...
            distance, dist_by_type = ImageVisualFeatures.calc_feature_distance(self, feature_to_compare, matches)
    ...
    def calc_feature_distance(self, feature_to_compare, matches, descriptor_match_normalize_factor=1):
        ...
        if self.feature_type == "SIFT":
            ...
            distance = ImageVisualFeatures.get_weighted_distance(norm_mean_descriptor_dist,norm_num_matches_dist,norm_scale_ratio_distance)
...
class PointFeature:
    def __init__(self, opencv_kp):
        self.size = opencv_kp.size
        self.angle = opencv_kp.angle
        self.location = opencv_kp.pt
        self.response = opencv_kp.response
